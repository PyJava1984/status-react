(ns status-im.ui.screens.wallet.choose-recipient.models
...
(defn toggle-flashlight [{:keys [db]}]
   ...
   (fn [{old-symbol :symbol :as old-transaction}]
     (let [symbol-changed? (not= old-symbol symbol)]
       (cond-> (assoc old-transaction :to address :to-name name :whisper-identity whisper-identity)
         ...
         (and gasPrice symbol-changed?) (assoc :gas-price (money/bignumber gasPrice))
         (and symbol (not gasPrice) symbol-changed?) (assoc :gas-price (ethereum/estimate-gas symbol)))))))
...
(defn fill-request-from-url [data origin {{:keys [network] :as db} :db}]
...
(defn fill-request-from-contact [{:keys [address name whisper-identity]} {db :db}]
